...
public class BuildNewPaarsnpResult implements Function<BuildPaarsnpResult.PaarsnpResultData, ResultJson> {

  private final Logger logger = LoggerFactory.getLogger(BuildNewPaarsnpResult.class);

  private final Map<String, AntimicrobialAgent> agents;

  public BuildNewPaarsnpResult(final Map<String, AntimicrobialAgent> agents) {
    this.agents = agents;
  }

  @Override
  public ResultJson apply(final BuildPaarsnpResult.PaarsnpResultData paarsnpResultData) {
...
//    final var resistanceSets = this.agents.keySet().stream().collect(Collectors.toMap(Function.identity(), (agent) -> new HashMap<String, SetResult>()));
    ...
    paarsnpResultData.searchResult.getSetResults()
        ...
        .forEach(setResult -> {
          ...
          final var convertedAcquired = setResult.getFoundMembers().stream().filter(setMember -> setMember.getVariants().isEmpty()).map(member -> new Determinant(member.getGene(), determinantClass)).collect(Collectors.toList());
          final var convertedVariants = setResult.getFoundMembers().stream().filter(setMember -> !setMember.getVariants().isEmpty()).flatMap(SetMember::toVariantNames).map(name -> new Determinant(name, determinantClass)).collect(Collectors.toList());
          ...
          setResult
              ...
              .forEach(phenotype -> {
                    ...
                    final var convertedAcquiredModifiers = modifiers.stream().filter(modifier -> modifier.getVariants().isEmpty()).map(modifier -> new Determinant(modifier.toName(), DeterminantClass.fromModifierEffect(modifier.getEffect()))).collect(Collectors.toList());
                    final var convertedVariantModifiers = modifiers.stream().filter(modifier -> !modifier.getVariants().isEmpty()).map(modifier -> new Determinant(modifier.toName(), DeterminantClass.fromModifierEffect(modifier.getEffect()))).collect(Collectors.toList());

                    phenotype.getProfile().forEach(amKey -> {
                      final var foundAcquired = new ArrayList<Determinant>(convertedAcquired.size() + convertedAcquiredModifiers.size());
                      ...
                      final var foundVariants = new ArrayList<Determinant>(convertedAcquired.size() + convertedAcquiredModifiers.size());
